...
/* swaps students iff the first payed more than the second */
  void swapIfPayedMore(student a, student b) { 
    if(paidByStudent(a) > paidByStudent(b)) {
      swapStudents(&a, &b);
    }
 }
...
void readInput() {
 ...
 void orderStudents() {
 swapIfPayedMore(firstStudent, secondStudent);
 swapIfPayedMore(secondStudent, thirdStudent);
 swapIfPayedMore(firstStudent, secondStudent);
} 
